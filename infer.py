"""Sentence-level rhetorical-role inference using the LegalSeg fine-tuned
InLegalBERT checkpoint (L-NLProc/LegalSeg_InLegalBERT, "current sentence
only" (i) variant), aggregated up to a per-paragraph majority label.
"""
import os
...
class RhetoricalRoleClassifier:
    ...
    def classify_sentences(self, sentences: list[str], batch_size: int = 16) -> list[dict]:
        """Return [{"label": str, "confidence": float}] aligned with `sentences`."""
        results = []
    ...
    def classify_paragraph(self, paragraph_text: str, batch_size: int = 16) -> dict:
        """Split a paragraph into sentences, classify each, and roll up a
        majority label for the paragraph as a whole."""
